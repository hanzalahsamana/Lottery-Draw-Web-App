    ...
    return (
        <div className="md:h-125 2xl:h-[1000px] flex flex-col md:flex-row items-center md:items-center justify-between px-4 sm:px-17 py-6 md:py-0 gap-8 md:gap-0">
            <div className="z-20 flex flex-col gap-1 md:gap-0 2xl:gap-4 mt-10 -mb-25 md:mb-0 md:mt-6 w-full md:w-max">
                ...
                <div className="flex flex-col gap-2.5 mt-6 2xl:scale-150 origin-top-left bg-[#0b12209c] w-full md:w-max ring-2 ring-white/15 text-white px-4 pt-2.5 pb-3.5 rounded-md text-[15px] 2xl:text-base">
                    {currentDraw?.drawNo ?
                    ...
                    {currentDraw?.status === 'waiting' ? (
                        ...
                        <div className='flex gap-2.5 max-w-[400px]  overflow-auto customScroll pb-2 -mb-1'>
                            {Array.from({
                                length: Math.max(0, currentDraw?.resultNo?.length || 0)
                            }).map((_, i) => (
                                <div key={i} className={` min-w-7 min-h-7 aspect-square flex items-center justify-center rounded-full ${true ? 'bg-gray-600' : ''}`}>
