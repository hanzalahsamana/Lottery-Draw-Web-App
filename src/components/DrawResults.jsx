...
const DrawResults = ({ secondsLeft, draws, metadata }) => {
    ...
    return (
        <AnimationWrapper className='bg-transparent w-full py-13.5 px-2.5 md:px-7.5 overflow-hidden'>
                ...
                <div className="w-full overflow-x-auto">
                    <div className="min-w-[800px] flex flex-col gap-2">
                        ...
                        <div className="grid grid-cols-6 justify-between pb-3.75 px-5 text-white/60 font-normal text-sm relative">
                            ...
                            <p className="flex-1 px-5 text-center text-nowrap">Time</p>
                        ...
                        {draws?.map((row, index) => {
                            ...
                            return (
                                <div
                                    key={index}
                                    className={`w-full h-12.5 rounded-lg px-5 text-[13px] text-white/70 grid grid-cols-6 items-center gap-2 min-w-[800px] ${index % 2 === 0 ? "bg-white/5" : "bg-black/0"
                                        } relative`}
                                >
                                    ...
                                    <div className="px-5 flex-1 text-nowrap text-center">{row?.speciaNo || "unknown"}</div>
                                </div>
                            );
                        })}
                    </div>
